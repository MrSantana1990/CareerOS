...
- [ ] **P0 — reautorizar o Gmail (URGENTE, achado em 13/09 durante a auditoria).** `google_mail_scheduler` está falhando continuamente há mais de 26h (`RefreshError: invalid_grant`, refresh token revogado/expirado) — último scan bem-sucedido em 2026-09-11 23:16 UTC, 159 tentativas falhas consecutivas até agora. Não corrigível por código, exige reautorização humana real (mesma classe do histórico "renovar sessão do InfoJobs"). Enquanto isso não for feito, qualquer resposta real de recrutador chegada depois de 11/09 23:16 UTC é invisível ao Core.
