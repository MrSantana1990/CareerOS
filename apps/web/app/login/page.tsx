...
export default function Login() {
  ...
  async function verifyCode(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); setLoading(true); setMessage(""); const form = new FormData(event.currentTarget);
    const response = await fetch("/api/auth/recovery/verify", { method: "POST", credentials: "same-origin", headers: { "content-type": "application/json" }, body: JSON.stringify({ email, code: form.get("code") }) });
  ...
  return <main className="login-shell"><section className="login-card"><div className="login-brand">H</div><p className="eyebrow">HELPSYSTEM CARREIRA</p><h1>{mode === "login" ? "Acesso ao portal" : "Recuperar acesso"}</h1><p>{mode === "login" ? "Entre com sua conta administrativa para continuar." : "Receba um código no e-mail administrativo e continue nesta tela."}</p>
    ...
    : <form onSubmit={verifyCode}><label>Código recebido<input name="code" type="text" inputMode="text" autoCapitalize="characters" autoComplete="one-time-code" minLength={8} maxLength={8} required /></label>{message && <div className="login-error" role="status">{message}</div>}<button type="submit" disabled={loading}>{loading ? "Validando…" : "Entrar com o código"}</button><button type="button" className="login-link" onClick={() => { setStep("request"); setMessage(""); }}>Solicitar outro código</button></form>}
